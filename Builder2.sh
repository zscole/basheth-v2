...
echo "deleted static-nodes.json"

i=1
while [ "$i" -le "$N" ]; do
	echo "in the while loop${i}"
...
do
  	...
  	cp -r /home/appo/ubiq-net-intelligence-api/ /home/appo/node$e/ubiq-net-intelligence-api/ >> /tmp/$(date "+%y.%m.%d").log
  	sed -i "s/\"INSTANCE_NAME\".*/\"INSTANCE_NAME\"\t\:\ \"node$e\",/g" /home/appo/node$e/ubiq-net-intelligence-api/app.json
  	cd /home/appo/node$e/
  	tar -cf ubiq-net-intelligence-api.tar ./ >> /tmp/$(date "+%y.%m.%d").log
  	#  rm -rf /home/appo/node$e/ubiq-net-intelligence-api/
	...
	expect_password "ssh -t -o StrictHostKeyChecking=no node$e tmux send-keys -t whiteblock 'gubiq\ --datadir\ /home/appo/node$e\ --nodiscover\ --maxpeers\ 50\ --targetgaslimit\ $gas\ --networkid\ $n\ --rpc\ --unlock \\\"$unlock\\\" --password /home/appo/node$e/passwd.file --etherbase `cat /home/appo/node$e/wallet` console' C-m"

	#Start the net inteligence API on all nodes
	expect_password "ssh -t -o StrictHostKeyChecking=no node$e cd /home/appo/node$e && tar -xf ubiq-net-intelligence-api.tar && cd /home/appo/node$e/ubiq-net-intelligence-api && pm2 start app.json && rm /home/appo/node$e/ubiq-net-intelligence-api.tar" >> /tmp/$(date "+%y.%m.%d").log
